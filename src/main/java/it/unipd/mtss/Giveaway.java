...
public class Giveaway {
    ...
    public Giveaway(List<Order> _inputOrdersList) throws NullPointerException{
        ...
        if(_inputOrdersList!=null)
            for(int j=0; j<_inputOrdersList.size(); j++){
                ...
                if(_inputOrdersList.get(j).getUser().isUnderage())                                                       //minorenne?
                    if(_inputOrdersList.get(j).getTime().isAfter(GIVEAWAYSTART)&&_inputOrdersList.get(j).getTime().isBefore(GIVEAWAYFINISH)){ //tra le 18 e le 19 ?
        ...
        else throw new NullPointerException("The list of order could not be found");
    }
    ...
    public boolean giveawaySelection(){
        if(!candidateOrders.isEmpty()){
            ...
            for(int i=0; i<Math.max(winners.size(), NUMBEROFWINNERS); i++){
                System.out.println(winners.get(i).getUser().getId()+"\t\t"+winners.get(i).getUser().getName()+"\n\n"+winners.get(i).getUser().getSurname());
            }
            return true;
        }
        return false;
